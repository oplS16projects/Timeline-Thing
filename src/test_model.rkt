...
(struct post (timeline id))
  (define sql3 (sqlite3-connect #:database 'memory #:mode 'read/write
                                ))
(sqlite3-available?)
...
(define (initialize-timeline! sql3)
  ;;
  (define the-timeline (timeline sql3))
;;  (unless (table-exists? db "posts")
  (define (serve req)
    (query-exec sql3
                "create table date (m varchar(12), d 
;;    (query-exec db
;;     (string-append
;;      "CREATE TABLE posts "
